...
export async function upsertAsset(
  db: D1Database,
  asset: Omit<AssetMetadata, "resolved_at">,
): Promise<void> {
  await db
    .prepare(
      `INSERT INTO assets
         (file_id, name, account_id, workspace_id, project_id, parent_folder_id,
          file_size, media_type, status, view_url, raw, resolved_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'))
...
export async function watchAsset(
  db: D1Database,
  asset: Omit<WatchedAsset, "watched_at" | "view_url">,
): Promise<void> {
  await db
    .prepare(
  ...
  await db
    .prepare(
      `INSERT INTO captured_comments
         (comment_id, file_id, parent_id, author_name, author_email, text, timecode, raw_payload)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)
